...
{
    internal partial class HotStringFactory : BaseRuleFactory
    {
        private readonly IEnumerable<string>? _names;
        private readonly IEnumerable<AutoKeyRule>? _hotStrings;

        public HotStringFactory()
        {
        ...
        private List<BaseRuleRecord> GetNameRulesFromConfiguration()
        {
            ...
            foreach (string s in _names)
            {
                ...
                {
                    if (_names.Count(x => x.StartsWith(s.Substring(0, i))) == 1)
                    {
                        var names = s.Split(' ');
                        rules.Add(new HotStringRuleRecord("фио" + s.Substring(0, i), s, false));
                        rules.Add(new HotStringRuleRecord("ф" + s.Substring(0, i), $"{names[0]} {names[1].FirstOrDefault()}.{names[2].FirstOrDefault()}.", false));
                        break;
                    }
                }
            }
