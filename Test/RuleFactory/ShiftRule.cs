...
namespace AutoKeyNetApp.RuleFactory
{
    internal class ShiftRule : BaseRuleFactory
    {
        ...
        public override List<BaseRuleRecord> Create()
        {
            ...
            {
                new HotKeyRuleRecord("{LSHIFT DOWN}", ChangeLanguageDown(en_US)), // Set text as underscore
                new HotKeyRuleRecord("{LSHIFT UP}", ChangeLanguage(en_US)), // Set text as underscore

                new HotKeyRuleRecord("{RSHIFT DOWN}", ChangeLanguageDown(ru_RU)), // Set text as underscore
                new HotKeyRuleRecord("{RSHIFT UP}", ChangeLanguage(ru_RU)) // Set text as underscore
            };
            return rules;
        }
        ...
        TimerCallback tm = new TimerCallback(SetFalse);
        ...
        private Action ChangeLanguageDown(string enUs)
        {
            return () =>
            {
                _isShiftDown = true;
        ...
        private static Action ChangeLanguage(string pwszKLID)
        {
            return () =>
            {
                if (_isShiftDown)
                {
                    Debug.WriteLine("ChangeLanguage");
